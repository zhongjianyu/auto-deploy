 ...
 * @作者：zhongjy
 *
 * @时间：2017年5月10日 下午9:28:06
 */
public class CustomJdbcTokenRepositoryImpl extends JdbcDaoSupport implements PersistentTokenRepository {
	public static final String CREATE_TABLE_SQL = "create table aut_remember_me (user_name varchar(64) not null, id varchar(64) primary key, token varchar(64) not null, last_used timestamp not null)";
	public static final String DEF_TOKEN_BY_SERIES_SQL = "select user_name,id,token,last_used from aut_remember_me where id = ?";
	public static final String DEF_INSERT_TOKEN_SQL = "insert into aut_remember_me (user_name, id, token, last_used) values(?,?,?,?)";
	public static final String DEF_UPDATE_TOKEN_SQL = "update aut_remember_me set token = ?, last_used = ? where id = ?";
